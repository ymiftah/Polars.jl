module Strings
    ...
    # next to each primal in the block above. `head`/`tail` are the exception, pulled out of the
    # macro block entirely (see the comment there), so they get their own `@curry ... fix2 = true`
    # here instead.
