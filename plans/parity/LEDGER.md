...
**Sweep complete. Batches 0-14 are all done** (see the batch order
...
| 10 | operations/frame_verbs.jl, reshape.jl, concat.jl, select_with_columns.jl, filter.jl | operations/test_pivot.py, test_unpivot.py, test_transpose.py, test_drop.py, test_rename.py, test_select.py, test_with_columns.py, test_filter.py, functions/test_concat.py, dataframe/test_vstack.py, dataframe/test_upsample.py | **done** — see `plans/parity/batch-10-frame-verbs.md`; 4 confirmed divergences flagged (`drop`'s `"*"` wildcard, `drop_nulls`'s empty-subset no-op, `concat` on a 0-column frame, `transpose` on a 0-row frame — the last a Rust-version-pinned difference, already correctly handled), plus 2 further no-Cargo-change gaps recorded (`drop`'s missing `strict` kwarg, no frame-level `drop_nans`) |
| 11 | operations/join.jl, group_by.jl, group_by_dynamic.jl | operations/test_join.py, test_join_asof.py, test_join_right.py, test_cross_join.py, test_group_by.py, test_group_by_dynamic.py, operations/rolling/test_rolling.py | **done** — see `plans/parity/batch-11-join-groupby.md`; 8 fixtures ported (join abort-safety, rightjoin coalesce ordering, group_by/group_by_dynamic abort-safety), 1 gap recorded (no join variant threads `maintain_order`) |
| 12 | datatypes/series.jl, binary.jl, dataframe/construction.jl, io.jl, describe.jl | series/test_series.py, series/test_getitem.py, series/test_to_list.py, dataframe/test_df.py, dataframe/test_shape.py, dataframe/test_describe.py, datatypes/test_binary.py, datatypes/test_null.py, constructors/test_constructors.py | **done** — see `plans/parity/batch-12-series-dataframe.md`; fixed a real bug (`describe`'s fractional-percentile labels collided), ported 9 fixtures, recorded 2 gaps (`describe` on a columnless frame doesn't raise; no `eq_missing`/`ne_missing`/`hash`) |
| 13 | lazyframe/scan_*.jl, sink_*.jl, collect_schema.jl, head.jl | io/test_csv.py, io/test_parquet.py, io/test_ipc.py, io/test_lazy_csv.py, io/test_lazy_parquet.py, io/test_lazy_ipc.py, io/test_scan_options.py, io/test_sink.py, lazyframe/test_collect_schema.py | **done** — see `plans/parity/batch-13-io-lazyframe.md`; fixed a real bug (`head`/`tail` crashed with a bare `InexactError` on negative `n`, now a clean `ArgumentError`), ported 3 fixtures |
| 14 | expr/selectors.jl, meta.jl, horizontal.jl, naming.jl, sample.jl, curried_forms.jl, misc.jl | operations/test_selectors.py, operations/namespaces/test_meta.py, expr/test_meta.py, operations/namespaces/test_name.py, functions/test_horizontal.py, operations/aggregation/test_horizontal.py, operations/test_random.py, functions/test_col.py, functions/test_nth.py | **done** — see `plans/parity/batch-14-selectors-misc.md`; 4 fixtures ported (nth indexing/duplicate, horizontal-reduction shape mismatch, by_name order, sample_n no-shuffle order preservation), 2 gaps recorded (Meta missing 5 methods, nth has no multi-arg form) — a third claimed gap (`sample` missing `shuffle`) was a false negative from grepping the wrong file, corrected. **This was the last unswept batch — sweep complete.** |
